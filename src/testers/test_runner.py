...
class UniversalTestRunner:
    ...
    TEST_FRAMEWORKS = {
        ...
        "javascript": {
            ...
            "mocha": {
                ...
                "command": ["npm", "test", "--", "--reporter", "json", "--reporter-options", "output={output}"],
    ...
    def run_tests(self) -> Dict[str, Any]:
        ...
        self.results["summary"] = {
            ...
            "frameworks_used": [
                f"{lang}_{fw}" for lang, fws in frameworks.items() 
                for fw, info in fws.items() if info["detected"]
            ]
        }
            ...
            for framework, config in frameworks.items():
                ...
                for indicator in config["indicators"]:
                    if "*" in indicator:
                        ...
                        if matches:
                            detected = True
                            detected_files.extend([str(m.relative_to(self.repo_path)) for m in matches[:5]])
                    else:
                ...
                if language in ["javascript", "typescript"] and (self.repo_path / "package.json").exists():
                    try:
                        with open(self.repo_path / "package.json", 'r') as f:
                            package_json = json.load(f)
                        
                        # Check devDependencies and dependencies
                        deps = {**package_json.get("devDependencies", {}), **package_json.get("dependencies", {})}
    ...
    def _run_framework_tests(self, language: str, framework: str, info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _execute_test_command(self, language: str, framework: str, command: List[str]) -> subprocess.CompletedProcess:
    ...
    def _parse_test_results(self, language: str, framework: str, result: subprocess.CompletedProcess, output_path: str) -> Dict[str, Any]:
        """Parse test results based on framework output format."""
        test_data = {
            "framework": framework,
            "language": language,
            "exit_code": result.returncode,
            "stdout": result.stdout[-5000:] if len(result.stdout) > 5000 else result.stdout,  # Limit output
        ...
        xml_files = list(self.repo_path.rglob("**/TEST-*.xml")) + list(self.repo_path.rglob("**/test-results/*.xml"))
        if xml_files:
            test_data["junit_results"] = self._parse_junit_xml(xml_files[0])
        
        return test_data
    
    def _extract_summary(self, results: Dict[str, Any], framework: str) -> Dict[str, int]:
