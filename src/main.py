...
from .analyzers.documentation_analyzer import DocumentationAnalyzer, ClaimVerifier
...
class VibeVerifier:
    ...
    def run_analysis(self) -> Dict[str, Any]:
        ...
        print(f"  Average complexity: {complexity_results['summary']['average_complexity']:.2f}")
        ...
        for category in ["linting", "type_checking", "security"]:
            for tool, results in static_results.get(category, {}).items():
        ...
        verified_count = 0
        for lang, contracts in verification_results.get("contracts", {}).items():
            if contracts:
...
Examples:
  ...
  vibe-verifier /path/to/repo --skip-tests --quick
    ...
    parser.add_argument(
        "--skip-verification",
        action="store_true",
        help="Skip formal verification phase"
    )
    
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Quick analysis (skip time-consuming checks)"
    )
