...
class TestVibeVerifierIntegration:
    ...
    def test_cli_basic(self, mock_verifier_class):
        ...
        mock_verifier_class.assert_called_once_with(
            "/test/path",
            {
                "output_format": "all",
                "skip_tests": False,
    ...
    def test_cli_with_config_file(self, mock_verifier_class, mock_open):
        ...
        mock_open.return_value.__enter__.return_value.read.return_value = json.dumps(
            {"output_format": "pdf", "save_raw_results": True}
        )
    ...
    def test_save_raw_results(self, sample_python_project, temp_dir, mock_subprocess_run):
        ...
        config = {"save_raw_results": True, "output_dir": str(temp_dir)}
...
"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            perf_claims = [
                claim
                for claims in results["documentation"]["claims_by_type"].values()
                for claim in claims
                if "performance" in claim.get("type", "")
                or "operations per second" in claim.get("text", "")
            ]
