...
from src.analyzers.documentation_analyzer import DocumentationAnalyzer, ClaimVerifier, Claim
...
""")
        ...
        for claim_type, claims in result["claims_by_type"].items():
            for claim in claims:
        ...
        assert any("always" in claim["text"] or "guaranteed" in claim["text"] 
    ...
    def test_verification_recommendations(self, sample_python_project):
        ...
        assert any(t in rec_types for t in ["unit_testing", "performance_testing", 
                                            "security_testing", "integration_testing"])
